...
export abstract class Formatter {
    ...
    protected async replaceLinkToCurrentFileInString(input: string): Promise<string> {
        ...
        while (LINK_TO_CURRENT_FILE_REGEX.test(output)) output = this.replacer(output, LINK_TO_CURRENT_FILE_REGEX, currentFilePathLink);
        ...
        while (VARIABLE_REGEX.test(output)) {
            ...
            if (variableName) {
                if (!this.getVariableValue(variableName)) {
                    ...
                    if (suggestedValues.length === 1) this.variables.set(variableName, await this.promptForVariable(variableName));
                    else this.variables.set(variableName, await this.suggestForValue(suggestedValues));
                }
    ...
    protected async replaceFieldVarInString(input: string) {
        ...
        while (FIELD_VAR_REGEX.test(output)) {
            ...
            if (variableName) {
                if (!this.getVariableValue(variableName)) {
                    this.variables.set(variableName, await this.suggestForField(variableName));
                }

                output = this.replacer(output, FIELD_VAR_REGEX, this.getVariableValue(variableName));
            } else {
                break;
            }
        }
        ...
        while (DATE_VARIABLE_REGEX.test(output)) {
            ...
            if (variableName && dateFormat) {
                if (!this.variables.get(variableName)) {
                    ...
                    if (parseAttempt) this.variables.set(variableName, parseAttempt.moment.format(dateFormat));
                    else throw new Error(`unable to parse date variable ${this.variables.get(variableName)}`);
                }
