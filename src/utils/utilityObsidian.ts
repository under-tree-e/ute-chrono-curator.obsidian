import type { App, TAbstractFile, WorkspaceLeaf, CachedMetadata } from "obsidian";
...
export async function replaceTemplaterTemplatesInCreatedFile(app: App, file: TFile, force = false) {
    const templater = getTemplater(app);

    if (templater && (force || !(templater.settings as Record<string, unknown>)["trigger_on_file_creation"])) {
...
export async function templaterParseTemplate(app: App, templateContent: string, targetFile: TFile) {
    ...
    return await (
        templater.templater as {
            parse_template: (opt: { target_file: TFile; run_mode: number }, content: string) => Promise<string>;
        }
...
export function excludeKeys<T extends object, K extends keyof T>(sourceObj: T, except: K[]): Omit<T, K> {
...
export function getChoiceType<T extends TemplateChoice | MultiChoice | CaptureChoice | MacroChoice>(choice: IChoice): choice is T {
    const isTemplate = (choice: IChoice): choice is TemplateChoice => choice.type === "Template";
    const isMacro = (choice: IChoice): choice is MacroChoice => choice.type === "Macro";
    const isCapture = (choice: IChoice): choice is CaptureChoice => choice.type === "Capture";
    const isMulti = (choice: IChoice): choice is MultiChoice => choice.type === "Multi";
